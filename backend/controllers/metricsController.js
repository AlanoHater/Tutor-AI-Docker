// controllers/metricsController.js - Métricas y analytics del sistema (VERSIÓN CORREGIDA)
...
const getMetricasGenerales = async (req, res) => {
  try {
    ...
    const [
      ...
      totalQuizzes,
      totalArchivos,
      profesoresActivos, // CORREGIDO: Ahora cuenta profesores activos correctamente
      usoPorMateria,
      quizzesPorTipo,
      actividadReciente
    ] = await Promise.all([
      ...
      // CORREGIDO: Profesores activos (todos los que tienen activo = 1)
      db.get("SELECT COUNT(*) as total FROM usuarios WHERE rol = 'profesor' AND activo = 1"),
    ...
    res.json({
      metricasGenerales: {
        ...
        profesoresActivos: profesoresActivos.total, // CORREGIDO: Ahora refleja correctamente
        porcentajeActivos,
